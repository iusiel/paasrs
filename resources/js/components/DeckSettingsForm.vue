    ...
    <form id="deckSettingsForm" @submit="submitForm">
        ...
        <div class="mb-3">
            <label
                ...
                >Number of cards per review</label
            >
            <input
                v-model="formFields.numberOfCardsPerReview.value"
                type="text"
            ...
            />
            <div class="form__error-message">
                {{ formFields.numberOfCardsPerReview.errorMessage }}
            </div>
        </div>
        <div class="mb-3">
            <label for="hard_interval" class="form-label form-label__required"
                >Hard interval (in days)</label
            >
            <input
                v-model="formFields.hardInterval.value"
                type="text"
                name="hard_interval"
                id="hard_interval"
                class="form-control"
            />
            <div class="form__error-message">
                {{ formFields.hardInterval.errorMessage }}
            </div>
        </div>
        <div class="mb-3">
            <label for="good_interval" class="form-label form-label__required"
                >Good interval (in days)</label
            >
            <input
                v-model="formFields.goodInterval.value"
                type="text"
                name="good_interval"
                id="good_interval"
                class="form-control"
            />
            <div class="form__error-message">
                {{ formFields.goodInterval.errorMessage }}
            </div>
        </div>
        <div class="mb-3">
            <label for="easy_interval" class="form-label form-label__required"
                >Easy interval (in days)</label
            >
            <input
                v-model="formFields.easyInterval.value"
                type="text"
                name="easy_interval"
                id="easy_interval"
                class="form-control"
...
export default {
    ...
    mounted() {
        if (typeof this.deck !== "undefined") {
            ...
            this.formFields.numberOfCardsPerReview.value =
                    ...
                    error.json().then((result) => {
                        ...
                        errors.forEach((fieldError) => {
                            ...
                            const convertedFieldName =
                                this.convertReturnFieldNameToFormFieldName(
                                    field
                                );

                            if (
                                typeof this.formFields[convertedFieldName] !==
                                "undefined"
                            ) {
                                this.formFields[
                                    convertedFieldName
